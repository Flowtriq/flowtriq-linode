...
## Flowtriq Agent Settings
#<UDF name="api_key" label="Flowtriq API Key" example="Get your API key from flowtriq.com/dashboard after adding a node">
#<UDF name="node_uuid" label="Flowtriq Node UUID" example="Get the Node UUID from flowtriq.com/dashboard after adding a node">
...
export MARKETPLACE_APP="apps/linode-marketplace-flowtriq"
...
EOF
  ...
  # ftagent-specific vars
  if [[ -n ${API_KEY} ]]; then
    echo "ftagent_api_key: ${API_KEY}" >> ${group_vars};
  fi

  if [[ -n ${NODE_UUID} ]]; then
    echo "ftagent_node_uuid: ${NODE_UUID}" >> ${group_vars};
  fi
